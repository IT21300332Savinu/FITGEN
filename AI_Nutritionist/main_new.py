...
def validate_meal_plan(req: ValidationRequest):
    # Hardcoded UID (temporary)
    fallback_uid = "sKr1Ay7QOIcys7kzE0lJYkiNbyG3"

    #  start with whatever came from frontend
    profile = req.plan.profile or {}

    #  fetch real profile from Firebase (merge)
    try:
        snap = db.child("users").child(fallback_uid).child("profile").get()
        fb_profile = snap.val() or {}
        profile.update(fb_profile)  # merge → Firebase data overwrites frontend 0s
    except Exception as e:
        print(f" Could not fetch real profile: {e}")

    #  derive conditions from the merged profile
    conds = _conditions_from_profile(profile, req.conditions)

    #  validate normally
    gpt_result = validate_with_gpt(req.plan, conds)
    return gpt_result or validate_with_rules(req.plan, conds)
...
def save_custom_meal_plan(plan: CustomMealPlanRequest):
    ...
    #  start with whatever frontend sent
    profile = plan.profile or {}

    # pull real Firebase profile and merge
    try:
        snap = db.child("users").child(fallback_uid).child("profile").get()
        fb_profile = snap.val() or {}
        profile.update(fb_profile)
    except Exception as e:
        print(f" Could not fetch Firebase profile: {e}")

    #  validate with correct merged data
    conds = _conditions_from_profile(profile, None)
    ...
    payload = {
        ...
        "profile": profile,  #  merged one (correct diseases)
        "note": plan.note or "",
        "meals": {
    ...
    res = db.child("custom_meal_plans").push(payload)
...
def suggest_meal(req: SuggestMealRequest):
    uid = req.uid

    # 1) Read profile
    snap = db.child("users").child(uid).child("profile").get()
    profile: Dict[str, Any] = snap.val()

    if not profile:
        raise HTTPException(status_code=404, detail=f"No profile found for uid '{uid}'")

    userData = pd.DataFrame([profile]).rename(columns=lambda c: c.replace("_", " "))

    predicted_calories = float(model.predict(userData)[0])

    suggestions_series = get_meal_plan_for_calorie(
        predicted_calories,
        dietary_preference=profile["Dietary_Preference"],
        budget_preference=profile["Budget_Preferences"],
    )
    ...
    for key, csv_col in mapping.items():
        ...
        meals_block[key] = {
            ...
            "ingredients_with_alternatives": alternatives
        }

    # 6) Save per-user suggestion
    payload = {
        "ts_ms": int(time.time() * 1000),
        "predicted_calories": predicted_calories,
        "profile": profile,      # saved as-is
        "meals": meals_block
    }
    push_result = db.child("users").child(uid).child("meal_suggestions").push(payload)
    firebase_id = push_result.get("name")

    REQUIRED_COLS = [
        "Age", "Gender", "Height", "Weight",
        "Activity Level", "Dietary Preference", "Budget Preferences",
        "Acne", "Diabetes", "Heart Disease", "Hypertension",
        "Kidney Disease", "Weight Gain", "Weight Loss"
    ]

    norm_profile = {c: (int(userData.at[0, c]) if c in {
        "Acne", "Diabetes", "Heart Disease", "Hypertension", "Kidney Disease", "Weight Gain", "Weight Loss"
    } else userData.at[0, c]) for c in REQUIRED_COLS}

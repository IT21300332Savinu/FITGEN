...
class BicepCurlDetector {
  ...
  static const double MAX_ELBOW_MOVEMENT =
      0.1; // Max elbow movement for good form
  ...
  double _calculateElbowAngle(
    ...
    double shoulderLength = math.sqrt(
  ...
  ExerciseState _analyzeMovement(double currentAngle) {
    ...
    double timeSinceLastChange =
        now.difference(_lastStateChange).inMilliseconds / 1000.0;
    ...
    switch (_currentState) {
      ...
      case ExerciseState.ascending:
        ...
        if (smoothedAngle > EXTENDED_ANGLE - ANGLE_THRESHOLD &&
            timeSinceLastChange > MIN_REP_TIME) {
  ...
  FormQuality _calculateFormQuality(double angle, PoseLandmark elbow) {
    ...
    // Check angle range - be more lenient
    if (angle > EXTENDED_ANGLE + 30 || angle < CURLED_ANGLE - 30) {
      formScore -= 20; // Reduced penalty for range
    }

    // Check elbow stability - be more lenient
    if (_elbowPositionHistory.length >= 3) {
      double elbowMovement = _getElbowMovementVariation();
      if (elbowMovement > MAX_ELBOW_MOVEMENT * 1.5) {
        // Increased tolerance
        formScore -= 15; // Reduced penalty
      }
    }

    // Check movement smoothness - be more lenient
    if (_angleHistory.length >= 3) {
      double smoothness = _getMovementSmoothness();
      if (smoothness > 30) {
        // Increased tolerance for variation
        formScore -= 10; // Reduced penalty
      }
    }

    // Check speed - only penalize extremely fast movements
    DateTime now = DateTime.now();
    double timeSinceLastChange =
        now.difference(_lastStateChange).inMilliseconds / 1000.0;
    if (_currentState != ExerciseState.ready && timeSinceLastChange < 0.2) {
      // Reduced time threshold
      formScore -= 10; // Reduced penalty
    }

    // More lenient thresholds
    if (formScore >= 85) return FormQuality.excellent;
    if (formScore >= 65) return FormQuality.good;
    if (formScore >= 45) return FormQuality.warning;
  ...
  double _getElbowMovementVariation() {
    ...
    double sum = 0;
    double mean =
        _elbowPositionHistory.reduce((a, b) => a + b) /
        _elbowPositionHistory.length;

...
class ShoulderPressDetector {
  ...
  bool detectShoulderPress(List<PoseLandmark> landmarks) {
    ...
    double leftShoulderElevation = _calculateShoulderElevation(leftShoulder, leftElbow);
    double rightShoulderElevation = _calculateShoulderElevation(rightShoulder, rightElbow);
    double avgShoulderElevation = (leftShoulderElevation + rightShoulderElevation) / 2;
    ...
    _analyzeForm(avgArmAngle, avgShoulderElevation, 
  ...
  double _calculateShoulderElevation(PoseLandmark shoulder, PoseLandmark elbow) {
    ...
    double angle = atan2(-deltaY, deltaX) * 180 / pi; // Negative Y because screen coordinates
  ...
  void _analyzeForm(double armAngle, double shoulderElevation,
    ...
    if (shoulderElevation < MIN_ELEVATION_ANGLE && _currentPhase == ShoulderPressPhase.bottom) {
    ...
    if (_currentPhase == ShoulderPressPhase.top) {
      ...
      if (!leftWristAboveElbow || !rightWristAboveElbow) {
        _formIssues.add('Press weights directly overhead - keep wrists above elbows');
        quality -= 0.2;
      }
    }
    ...
    if (_currentPhase == ShoulderPressPhase.top && armAngle < TOP_ARM_ANGLE - ANGLE_THRESHOLD) {
    ...
    if (handWidthRatio < 0.8 || handWidthRatio > 1.5) {
      _formIssues.add('Adjust grip width - hands should be about shoulder-width apart');
      quality -= 0.1;
    }

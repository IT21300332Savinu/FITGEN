...
extension UserModelApiExtension on UserModel {
  Map<String, dynamic> toMealSuggestionJson({
    ...
    bool has(String name) => selectedConditions.any(
            (c) => c.toLowerCase().replaceAll(' ', '') == name.toLowerCase().replaceAll(' ', ''));
    ...
    return {
      ...
      "Budget_Preferences": budgetPreference,
      "Acne": has("Acne") ? 1 : 0,
      "Diabetes": has("Diabetes") ? 1 : 0,
      "Heart_Disease": has("Heart Disease") ? 1 : 0,
      "Hypertension": has("Hypertension") ? 1 : 0,
      "Kidney_Disease": has("Kidney Disease") ? 1 : 0,
      "Weight_Gain": has("Weight Gain") ? 1 : 0,
      "Weight_Loss": has("Weight Loss") ? 1 : 0,
    };
  }
}

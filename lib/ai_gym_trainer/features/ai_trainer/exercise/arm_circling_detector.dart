...
class ArmCirclingDetector {
  ...
  void _analyzeForm(
    ArmPosition leftArmPos,
    ArmPosition rightArmPos,
    ...
    double leftElbowDistance = sqrt(
      leftElbowX * leftElbowX + leftElbowY * leftElbowY,
    );

    double rightElbowX = rightElbow.x - rightShoulder.x;
    double rightElbowY = rightElbow.y - rightShoulder.y;
    double rightElbowDistance = sqrt(
      rightElbowX * rightElbowX + rightElbowY * rightElbowY,
    );
    ...
    if (leftExtensionRatio < 0.6 || rightExtensionRatio < 0.6) {
      _formIssues.add(
        'Keep your arms more extended - don\'t bend elbows too much',
      );
      quality -= 0.2;
    }
    ...
    if (_leftArmPositions.length > 5) {
      double avgLeftRadius =
          _leftArmPositions.map((pos) => pos.distance).reduce((a, b) => a + b) /
          _leftArmPositions.length;

      double avgRightRadius =
          _rightArmPositions
              .map((pos) => pos.distance)
              .reduce((a, b) => a + b) /
          _rightArmPositions.length;

      // Check if circles are too small or inconsistent
      if (avgLeftRadius < MIN_CIRCLE_RADIUS ||
          avgRightRadius < MIN_CIRCLE_RADIUS) {
    ...
    if (_leftArmAngles.length > 5 && _rightArmAngles.length > 5) {
      double phaseDifference =
          (_leftArmAngles.last - _rightArmAngles.last).abs();
      if (phaseDifference > pi / 2) {
        // More than 90 degrees out of sync
